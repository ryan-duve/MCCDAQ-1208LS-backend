...
int main (int argc, char **argv)
{
  ...
  if ((interface = PMD_Find_Interface(&hid, 0, MINILAB1008_PID)) < 0) {
    fprintf(stderr, "miniLAB 1008 not found.\n");
    exit(1);
  } else {
    printf("miniLAB 1008 Device is found! interface = %d\n", interface);
  }
  ...
  while(1) {
    ...
    case 'd':
      ...
      do {
        ...
        scanf("%x", &temp);
        usbDOut_miniLAB1008(hid, DIO_PORTA, (__u8)temp);
        usbDIn_miniLAB1008(hid, DIO_PORTB, &input);
        usbDOut_miniLAB1008(hid, DIO_AUXPORT, (__u8)temp);
