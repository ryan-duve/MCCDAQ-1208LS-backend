...
int main (int argc, char **argv) {
  ...
  hid_return ret;
  int interface;
  float table_AIN[NGAINS_1616FS][NCHAN_1616FS][2];
  ...
  for ( i = 0; i <= 6; i++ ) {
    if ((interface = PMD_Find_Interface(&hid[i], i, USB1616FS_PID)) < 0) {
      fprintf(stderr, "USB 1616FS not found.  i = %d\n", i);
      exit(1);
    } else {
  ...
  usbBuildCalTable_USB1616FS(hid[0], table_AIN);
  ...
  while(1) {
	...
	for (i = 0; i < 4; i++) {
  	  usbDConfigBit_USB1616FS(hid[0], i, DIO_DIR_OUT);
	  usbDOutBit_USB1616FS(hid[0], i, 0x0);
	  usbDConfigBit_USB1616FS(hid[0], i+4, DIO_DIR_IN);
	  usbDInBit_USB1616FS(hid[0], i+4);
	}
	temp = 0x0;
	do {
	  ...
	  for (i = 0; i < 4; i++) {
	    usbDOutBit_USB1616FS(hid[0], i, (temp & 0x1));
	    temp >>= 0x1;
	  }
	  temp = 0x0;
	  // read in the nibble on bits 4-7
	  for (i = 0; i < 4; i++) {
	    temp |= (usbDInBit_USB1616FS(hid[0], i+4) << i);
	  }
	  printf("your value = %x\n", temp);
	} while (toContinue());
	break;
      case 'c':
        ...
        usbDConfigBit_USB1616FS(hid[0], 0, DIO_DIR_OUT);
        sleep(1);
        flag = fcntl(fileno(stdin), F_GETFL);
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
        do {
          usbDOutBit_USB1616FS(hid[0], 0, 1);
	  usleep(300000);
          usbDOutBit_USB1616FS(hid[0], 0, 0);
	  printf("Counter = %d\n",usbReadCounter_USB1616FS(hid[0]));
        } while (!isalpha(getchar()));
        fcntl(fileno(stdin), F_SETFL, flag);
        break;
      case 'g':
        printf("Serial Number = %s\n", PMD_GetSerialNumber(hid[0]));
        break;
      case 'I':
	...
	for ( i = 0; i < 1024; i++ ) {  // load data with known value
	  in_data[i] = 0xbeef;
        ...
        usbAInScan_USB1616FS(hid, 0, 0, count, &freq, options, in_data, table_AIN, gainArray);
	...
	break;
      case 'i':
        ...
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
        do {
	  svalue = usbAIn_USB1616FS(hid[0], channel, gain, table_AIN);
