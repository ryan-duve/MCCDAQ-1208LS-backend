...
void usbDConfigPort_USB1408FS(HIDInterface* hid, __u8 port, __u8 direction)
{
...
/* reads digital port  */
void usbDIn_USB1408FS(HIDInterface* hid, __u8 port, __u8* din_value)
{
  __u8 reportID = DIN;
  int nread;
...
/* writes digital port */
void usbDOut_USB1408FS(HIDInterface* hid, __u8 port, __u8 value)
{
  struct t_write_port {
    __u8 reportID;
    __u8 port;
...
void usbAOut_USB1408FS(HIDInterface* hid, __u8 channel, __u16 value)
{
  value <<= 0x4;
...
int usbAOutScan_USB1408FS(HIDInterface* hid[], __u8 lowchannel, __u8 highchannel,
			  __u32 count, float *frequency, __u16 data[], __u8 options)
{
  ...
  *frequency = 10e6/((1<<scanReport.prescale)*preload);
  ...
  while (num_samples > 0) {
    byte = 0x0;
    do {
      nread = usb_interrupt_read(hid[0]->dev_handle, USB_ENDPOINT_IN | 1, (char *)&byte, 1, 30);
      if (nread < 0) {
        perror("usbAOutScan_USB1408FS");
        return -1;
      }
...
void usbAOutStop_USB1408FS(HIDInterface* hid)
{
...
signed short usbAIn_USB1408FS(HIDInterface* hid, __u8 channel, __u8 range)
{
  enum Mode mode;
  int nread;
  ...
  if (nread != sizeof(ain)) {
    printf("Error in usbAIn_USB1408FS. nread = %d\n", nread);
  }
...
void usbAInStop_USB1408FS(HIDInterface* hid)
{
  __u8 reportID = AIN_STOP;
  PMD_SendOutputReport(hid, AIN_STOP, &reportID, sizeof(reportID), FS_DELAY);
}

int usbAInScan_USB1408FS(HIDInterface* hid[], __u8 lowchannel, __u8 highchannel, __u32 count,
			  float *frequency, __u8 options, __s16 sdata[])
{
  ...
  *frequency = 10.e6/(preload*(1<<arg.prescale));
  ...
  while (num_samples > 0) {         
    nread = usb_interrupt_read(hid[pipe]->dev_handle, USB_ENDPOINT_IN |(pipe+2), (char *) &data, sizeof(data), FS_DELAY);
    if (nread < 0) {
      perror("usbAInScan_USB1408FS");
      return -1;
    }
    if (num_samples > 31) {
      for ( k = 0; k < 31;  k++ ) {
...
int usbAInScan_USB1408FS_SE(HIDInterface* hid[], __u8 lowchannel, __u8 highchannel, __u32 count,
			  float *frequency, __u8 options, __s16 sdata[])
{
  int num_samples;
  int i, k;
  ...
  *frequency = 10.e6/(preload*(1<<arg.prescale));
  ...
  usbALoadQueue_USB1408FS(hid[0], nchan, chan, gains);
  PMD_SendOutputReport(hid[0], AIN_SCAN, (__u8 *) &arg, sizeof(arg), FS_DELAY);
  i = 0;
  pipe = 1;   // Initial Enpoint to receive data.
  ...
  */
  ...
  while ( num_samples > 0 ) {
    usb_interrupt_read(hid[pipe]->dev_handle, USB_ENDPOINT_IN | (pipe+2), (char *) &data, sizeof(data), FS_DELAY);
    if (num_samples > 31) {
      for ( k = 0; k < 31;  k++ ) {
...
void usbALoadQueue_USB1408FS(HIDInterface* hid, __u8 num, __u8 chan[], __u8 gains[])
{
  struct t_aLoadQueue {
    __u8 reportID;
    __u8 num;        // number of channel/gain pairs to follow (max 8).
    __u8 gains[16];  // channel/gain pairs.
  } aLoadQueue;
  int i;
...
void usbInitCounter_USB1408FS(HIDInterface* hid)
{
  __u8 reportID = CINIT;
  PMD_SendOutputReport(hid, CINIT, (__u8*) &reportID, sizeof(reportID), FS_DELAY);
}

__u32 usbReadCounter_USB1408FS(HIDInterface* hid)
{
   __u32 value;
  struct t_counter {
    __u8 reportID;
...
void usbBlink_USB1408FS(HIDInterface* hid)
{
...
int usbReset_USB1408FS(HIDInterface* hid)
{
   __u8 reportID = RESET;
...
void usbSetTrigger_USB1408FS(HIDInterface* hid, __u8 type)
{
  __u8 cmd[2];
...
void usbSetSync_USB1408FS(HIDInterface* hid, __u8 type)
{
  __u8 cmd[2];
...
__u16 usbGetStatus_USB1408FS(HIDInterface* hid)
{
  __u16 status;
...
void usbReadMemory_USB1408FS( HIDInterface* hid, __u16 address, __u8 count, __u8 memory[])
{
...
int usbWriteMemory_USB1408FS(HIDInterface* hid, __u16 address, __u8 count, __u8* data)
{
...
void usbGetAll_USB1408FS(HIDInterface* hid, getAllValues* value)
{
  int i;
  struct t_getAll {
    __u8 reportID;
  ...
  for (i = 0; i < 4; i++) {
    value->ref_Low[i] =  (__s16) (getAll.values[2*i] + (getAll.values[2*i+1] << 0x8));
  }
  for (i = 0; i < 4; i++) {
    value->ref_High[i] =  (__s16) (getAll.values[8 + (2*i)] + (getAll.values[8 + (2*i+1)] << 0x8));
  }
  for (i = 0; i < 8; i++) {
    value->se[i] =  (__s16) (getAll.values[16 + (2*i)] + (getAll.values[16 + (2*i+1)] << 0x8));
  }
