...
start:
  ...
  usbBuildCalTable_USB1608FS(hid[0], table_AIN);
  // print the table
  for (i = 0; i < NGAINS_1608FS; i++) {
  ...
  while(1) {
	...
	in_data[i] = 0xbeef;
      ...
      usbAInScan_USB1608FS(hid, 0, 0, count, &freq, options, in_data, table_AIN, gainArray);
    ...
    case 'i':
	  ...
	  break;
        ...
        do {
          usbDOut_USB1608FS(hid[0], 0);
	  sleep(1);
	  svalue = usbAIn_USB1608FS(hid[0], channel, gain, table_AIN);
