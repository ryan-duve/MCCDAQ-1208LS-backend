...
int usbAInScanRead_USB2600(usb_dev_handle *udev, int nScan, int nChan, __u16 *data)
{
  ...
  if (ret != nbytes) {
    printf("usbAInScanRead_USB2600: error in usb_bulk_read.  ret = %d\n", ret);
  }
...
void usbTimerPeriodW_USB2600(usb_dev_handle *udev, __u8 timer, __u32 period)
{
  ...
  if (usb_control_msg(udev, requesttype, TIMER_PERIOD, period, timer, NULL, 0x0, HS_DELAY) < 0) {
...
{
  __u8 requesttype = (HOST_TO_DEVICE | VENDOR_TYPE | DEVICE_RECIPIENT);
  usb_control_msg(udev, requesttype, TIMER_PULSE_WIDTH, pulseWidth, timer, NULL, 0x0, HS_DELAY);
}
...
{
  __u8 requesttype = (HOST_TO_DEVICE | VENDOR_TYPE | DEVICE_RECIPIENT);
  usb_control_msg(udev, requesttype, TIMER_COUNT, count, timer, NULL, 0x0, HS_DELAY);
}
...
void usbTimerDelayW_USB2600(usb_dev_handle *udev, __u8 timer, __u32 delay)
{
  ...
  */
  __u8 requesttype = (HOST_TO_DEVICE | VENDOR_TYPE | DEVICE_RECIPIENT);
  usb_control_msg(udev, requesttype, TIMER_START_DELAY, delay, timer, NULL, 0x0, HS_DELAY);
}
